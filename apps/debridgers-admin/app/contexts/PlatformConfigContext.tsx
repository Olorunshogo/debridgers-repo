 ...
 * `default_tier_one_per_package_kobo`, `default_tier_two_per_package_kobo` and `default_delivery_cap_over_base_kobo` are defaults only: the taper and ceiling are per zone, so a quote for a specific delivery must read them off that zone rather than from here.
 */
...
/*
 * The running free-delivery campaign, or null when none is. Served publicly so the shop and the zone picker can announce it before anyone signs in.
...
/*
 * One conversion, in one place. Every consumer downstream works in naira and never has to ask which unit it is holding, which is the mistake that has already produced a 100x transfer bug and a 100x refund bug in this codebase.
 */
 ...
 * Both are exposed deliberately, because a stored percentage being multiplied as-is instead of divided down first already caused a real bug server-side, so no call site should have to remember which form it received.
 * `pricing` is null until the first response lands, and null if it failed.
 * It is deliberately not defaulted, since a pricing calculator that silently quotes against invented fee rules is worse than one that refuses to render.
 * `deliveryPromotion` is the running free-delivery campaign, or null.
 * It is null both when no campaign is running and before the first response lands, so gate on `isLoading` before concluding there is no promotion.
 ...
 * `error` is set when the fetch failed, meaning the values below are defaults rather than live data.
 */
...
/*
 * Used only until the first response lands, and only so the shape is never undefined. Anything user-facing should gate on `isLoading` instead of rendering these.
 */
const FALLBACK = {
  ...
  useEffect(() => {
    ...
    apiFetch<PublicConfigResponse>("/config/public")
      ...
      .catch(() => {
        if (cancelled) return;
        /*
         * Deliberately not silent. A wrong commission rate shown to an agent is a promise about their income, so callers are told the number is not live rather than being handed a plausible-looking default.
         */
