-- Baseline schema.
--
-- Generated by drizzle introspect, which emits the body inside a block comment
-- as a safety measure. It was committed still commented, so this migration
-- created nothing and every migration after it failed on missing tables: a
-- fresh database could not be provisioned at all. Uncommented deliberately.
--
-- The explicit btree operator classes it emitted are removed too: it typed a
-- boolean column as int4_ops, which Postgres rejects outright, so the baseline
-- died part way through its indexes. Every one of them was the default for its
-- column type anyway, and 0001 recreates several of these indexes without them.
