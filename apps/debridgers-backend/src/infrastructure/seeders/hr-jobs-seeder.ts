...
const OPEN_JOBS = [
  {
    ...
    description:
      "Build and maintain Debridgers web apps (buyer, agent, admin, HR) with React Router, shared UI packages, and our design system.",
    requirements:
  ...
  },
  {
    ...
    location: "Kaduna (hybrid)",
    description:
      "Own NestJS APIs, Drizzle/Postgres schemas, payments, and HR/recruitment services that power Debridgers operations.",
    requirements:
...
async function main() {
  ...
  for (const job of OPEN_JOBS) {
    const [existing] = await db
      .select({ id: schema.hrJobPostings.id })
      .from(schema.hrJobPostings)
      .where(
        and(
          eq(schema.hrJobPostings.title, job.title),
    ...
    await db.insert(schema.hrJobPostings).values({
      ...job,
      status: "open",
      created_by: admin.id,
    });
  ...
  console.log(`HR jobs seed done (${inserted} new)`);
  await pool.end();
}
