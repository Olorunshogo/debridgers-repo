...
export class HrPeopleService {
  ...
  async listDirectory(opts: {
    ...
    const isHrAdmin = opts.actor.role === "admin";

    const conditions = [isNull(schema.hrEmployees.deleted_at)];
    ...
    if (!isHrAdmin) {
      ...
      if (opts.actor.role === "employee") {
        conditions.push(
          or(
            eq(schema.hrEmployees.user_id, opts.actor.sub),
            eq(schema.hrEmployees.manager_user_id, opts.actor.sub),
            eq(schema.hrEmployees.status, "active"),
          )!,
        );
      }
    }
    ...
    if (opts.department) {
      conditions.push(eq(schema.hrEmployees.department, opts.department));
    }
    if (opts.status) {
      conditions.push(
        eq(
          schema.hrEmployees.status,
  ...
  async getEmployeeProfile(employeeUserId: number, actor: JwtPayload) {
    const isHrAdmin = actor.role === "admin";
  ...
  async listMyLeave(actor: JwtPayload) {
    return this.db
      ...
      .where(
        and(
          eq(schema.hrLeaveRequests.employee_user_id, actor.sub),
          isNull(schema.hrLeaveRequests.deleted_at),
        ),
      )
      .orderBy(desc(schema.hrLeaveRequests.created_at));
  }
  ...
  async listLeaveForReview(actor: JwtPayload) {
    ...
    if (isHrAdmin) {
      return this.db
        .select()
        .from(schema.hrLeaveRequests)
        .where(isNull(schema.hrLeaveRequests.deleted_at))
        .orderBy(desc(schema.hrLeaveRequests.created_at));
    }
    ...
    return this.db
      ...
      .where(
        and(
          sql`${schema.hrLeaveRequests.employee_user_id} in (${sql.join(
  ...
  async reviewLeave(
    ...
    const [updated] = await this.db
      ...
      .where(eq(schema.hrLeaveRequests.id, leaveId))
      .returning();
    ...
    if (nextStatus === "approved") {
      await this.db
        ...
        .where(eq(schema.hrEmployees.user_id, leave.employee_user_id));
    }
  ...
  async saveWorkReport(actor: JwtPayload, dto: UpsertWorkReportDto) {
    ...
    const [row] = await this.db
      .insert(schema.hrWorkActivityReports)
      .values({
        employee_user_id: actor.sub,
  ...
  async updateWorkReport(
    ...
    const [updated] = await this.db
      .update(schema.hrWorkActivityReports)
      .set({
        period_type: dto.period_type,
        period_start: new Date(dto.period_start),
        period_end: new Date(dto.period_end),
      ...
      .where(eq(schema.hrWorkActivityReports.id, reportId))
      .returning();
  ...
  async listMyWorkReports(actor: JwtPayload) {
    return this.db
      ...
      .where(
        and(
          eq(schema.hrWorkActivityReports.employee_user_id, actor.sub),
          isNull(schema.hrWorkActivityReports.deleted_at),
        ),
      )
      .orderBy(desc(schema.hrWorkActivityReports.created_at));
  }
  ...
  async listWorkReportsForReview(actor: JwtPayload) {
    ...
    if (isHrAdmin) {
      return this.db
        .select()
        .from(schema.hrWorkActivityReports)
        .where(isNull(schema.hrWorkActivityReports.deleted_at))
        .orderBy(desc(schema.hrWorkActivityReports.created_at));
    }
    ...
    return this.db
      ...
      .where(
        and(
          sql`${schema.hrWorkActivityReports.employee_user_id} in (${sql.join(
            teamIds.map((id) => sql`${id}`),
            sql`, `,
          )})`,
          isNull(schema.hrWorkActivityReports.deleted_at),
  ...
  private async requireEmployeeRow(userId: number) {
    const [row] = await this.db
      ...
      .where(
        and(
          eq(schema.hrEmployees.user_id, userId),
  ...
  private async requireLeave(id: number) {
    const [leave] = await this.db
      ...
      .where(
        and(
          eq(schema.hrLeaveRequests.id, id),
          isNull(schema.hrLeaveRequests.deleted_at),
        ),
      )
      .limit(1);
  ...
  private async requireWorkReport(id: number) {
    const [report] = await this.db
      ...
      .where(
        and(
          eq(schema.hrWorkActivityReports.id, id),
          isNull(schema.hrWorkActivityReports.deleted_at),
        ),
      )
      .limit(1);
  ...
  private async directReportUserIds(managerUserId: number): Promise<number[]> {
    const rows = await this.db
      ...
      .where(
        and(
          eq(schema.hrEmployees.manager_user_id, managerUserId),
          isNull(schema.hrEmployees.deleted_at),
        ),
      );
