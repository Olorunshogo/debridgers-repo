...
/*
 * The app's dialog registry: every key that triggerDialog can open, mapped to a lazy import so a dialog's code is only downloaded when first opened.
 * To add a dialog: build the presentation component in packages/ui-web (pure props in, no data fetching or routing knowledge), build a thin glue component in app/components/dialogs/ that wires useDialog() and useDialogSubmission() around it, then add the glue component here and open it with `const { triggerDialog } = useDialog(); triggerDialog('YOUR_KEY', { ...props })`.
...
export const DIALOG_REGISTRY: DialogRegistry = {
  ...
  CHECKOUT_GATE: () => import("../components/dialogs/CheckoutGateDialog"),
