 ...
 * `default_tier_one_per_package_kobo`, `default_tier_two_per_package_kobo` and `default_delivery_cap_over_base_kobo` are defaults only.
 * The taper and ceiling are per zone, so a quote for a specific delivery must read them off that zone rather than from here.
 */
...
/*
 * commissionPercent and commissionRate are both exposed deliberately: the API serves a PERCENTAGE (5), not a fraction (0.05), and the same ambiguity already caused a real bug server-side, where a stored percentage would have been multiplied as-is rather than divided down first.
 * No call site should have to remember which form it received.
 * pricing is null until the first response lands and null if it failed - deliberately not defaulted, since a pricing calculator that silently quotes against invented fee rules is worse than one that refuses to render.
 ...
 * isLoading stays true until the first fetch settles, so callers can hold off on rendering a number rather than flashing a placeholder that is wrong.
