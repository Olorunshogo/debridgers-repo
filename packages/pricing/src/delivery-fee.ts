 ...
 * costs, not just what the trip costs. The rates below are the Kaduna South
 * ones and are the fallback for a zone whose own rates are not set.
 */

// === The taper band
//
// The taper is what makes packages 7 and up cost less each than packages 3 to
// 6. It exists because the cost of a delivery is the trip, not the bag: a
// vehicle carrying one package costs very nearly what the same vehicle carrying
// six costs. A flat per-package rate over-charges exactly the large B2B order
// the business depends on, so the schedule has to descend.
//
// Two rates therefore have to hold one invariant, at every zone:
//
//     tier two < tier one
//
// The locked schedule in docs/business/BusinessModel.md descends steeply, from
// 700 naira to 400 for Kaduna South and 1,000 to 600 for Chikun. We run a
// shallower version of the same shape, mapped into a narrower operating band,
// until a second delivery cost measurement lands.
//
// The mapping is linear, so it preserves both the descent within a zone and the
// ordering between zones. Nothing is clamped: clamping collapsed Chikun's two
// rates onto the same number and produced a flat schedule, which is the defect
...
/*
 * The band the locked schedule is mapped onto. These two numbers are the only
 * control, and reverting is one edit:
 *
 ...
 *
 * Setting the band to the locked range makes intoTaperBand the identity
 * function and restores the locked schedule exactly. Setting both endpoints
 * equal removes the taper. Nothing else changes in either direction, and the
 * locked figures stay above so the derivation is never lost.
 *
 * Zone rates live in the database and are set from this same mapping by
 * migration 0024, so the fallbacks here and the rows there cannot disagree.
 */
export const TAPER_BAND_MIN_KOBO = 50_000;
export const TAPER_BAND_MAX_KOBO = 60_000;

/**
 * Maps a locked per-package rate into the operating band, rounded to the
 * nearest 500 kobo so the resulting naira figures stay legible on an invoice.
 *
 * Order-preserving, so a rate that was higher than another before the mapping
 * is still higher after it. That is what keeps the taper a taper.
 */
 ...
 * browser and must not depend on either one's error type.
 *
 * Nothing blocks a buyer on this. The message is written for an operator: it
 * reaches the buyer admin, who batches the drop with another in the same zone
 * so the trip is shared. A buyer who wants one keg of oil gets one keg of oil.
 */
...
export interface OrderTotals extends DeliveryFeeBreakdown {
  ...
  /**
   * True when the basket is below the delivery minimum.
   *
   * Reported, never enforced against the buyer. Refusing a single keg of oil to
   * protect a per-drop margin is the wrong trade: the buyer feels the rule, and
   * the rule is ours, not theirs. The trip cost is recovered by batching the
   * drop with another in the same zone, which is an operational answer, so this
   * flag exists to reach the person who does the batching.
   *
   * See `minimumOrderShortfallKobo` for how far below it sits.
   */
  belowMinimumOrder: boolean;
  /**
   * How far the basket is below the naira minimum, in kobo, or 0 when it is
   * not below it. The package shortfall is not money and is not folded in here.
   */
