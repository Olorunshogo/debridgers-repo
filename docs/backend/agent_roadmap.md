...
| 3.5 | 🔜 Commission split model: agencies vs individual agents (flat 30% vs configurable per agency)        | Config table needed (`agency_commission_rates`)                 |
...
| A4  | **Commission rate API** — hardcoded 30%, no way to change without code                                   | S     | `GET/PUT /admin/payments/commission-config` (zod-validated), `payments.config` table, env fallback                                                                     |
...
| #   | Item                                                                                                            | Size | Deliverable                                                                                                          |
| --- | --------------------------------------------------------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------- |
| B1  | **Admin commission audit API** — agents suspicious of their 30%, need transparent math                          | M    | `GET /admin/agents/:id/commissions/:id/audit`: return { report_id, amount_collected, commission_rate, calculated }   |
