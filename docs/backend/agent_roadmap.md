...
- ✅ Products page: agent sees available stock (title, unit price from the catalogue, availability)
...
| 3.5 | 🔜 Commission split model: agencies vs individual agents (flat per-package amount vs configurable per agency) | Config table needed (`agency_commission_rates`)                 |
...
| A4  | ✅ **Commission rate API** — done. Corrected 2026-08-29: commission is a flat naira amount per package, banded by product type (beans ₦1,200, rice ₦1,000, oil ₦700, else ₦400) in `agent-commission.ts`, not the `agent_commission_rate` percentage setting this row previously described | S     | `GET/PUT /admin/payments/commission-config` (zod-validated), `payments.config` table, env fallback                                                                     |
...
| #   | Item                                                                                                            | Size | Deliverable                                                                                                          |
| --- | --------------------------------------------------------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------- |
| B1  | **Admin commission audit API** — agents suspicious of their flat per-package commission, need transparent math  | M    | `GET /admin/agents/:id/commissions/:id/audit`: return { report_id, amount_collected, commission_rate, calculated }   |
