...
**Staffing is admin-only**, gated by desk. There is no `hr` / `hiring_manager` role in the `UserRole` enum - careers is a public subdomain app, not a role - but admin sub-accounts are still scoped by an `admin_desk` column (`"buyer" | "agent" | "hr"`), and staffing routes require the `"hr"` desk specifically.

- Super admin (`admin_tier: "super"`) can staff careers.
- Invited sub with `admin_desk: "hr"` can staff careers (same 2-header auth as other subs).
- Buyer / agent desks cannot call `/careers/admin/*` (403 from `AdminDeskGuard`).

Auth stack on authenticated careers admin routes:

`AuthGuard → AdminKeyGuard → RolesGuard → AdminDeskGuard` + `@AdminDesks("hr")`

- Super browser: JWT only.
- Sub (`hr` desk): JWT **plus** `X-Admin-Key` ∈ `SUPER_ADMIN_KEY_1|2` and `X-Admin-Tier: sub`. Set `VITE_ADMIN_SHARED_KEY` in the careers app to the same value.
- Applicants and employees pass through `AdminKeyGuard` (non-admin short-circuit) into `RolesGuard`.

Invite a careers admin from the main admin app: **Admin Invites → Desk = HR**.

**Naming note:** the desk value is still the literal string `"hr"` in code (`admin_desk`, `@AdminDesks("hr")`, the admin-invite dropdown) - it predates the app rename and was not renamed alongside it. Worth deciding deliberately whether to rename this desk to `"careers"` for consistency (touches `AdminDesk` type, `jwt.type.ts`, `admin-invite.controller/service.ts` + its spec, every `@AdminDesks("hr")` in `careers.controller.ts`, and the invite dropdown in `debridgers-admin`), or leave it as an internal desk label distinct from the public-facing app name.
